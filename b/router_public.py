...
def format_plan_info(code: str) -> str:
    """
    Продающее описание тарифа.
    Максимум конкретной ценности, минимум технической сухости.
    """

    plan = cfg.plans[code]

    # Выгоды — эмоциональные триггеры (можно расширять)
    BENEFITS = {
        "pro_lite": [
            "Мгновенные ответы без задержек",
            "Редактирование фото и генерация изображений",
            "Идеально для повседневных задач",
            "Оптимальное сочетание цены и возможностей"
        ],
        "pro_plus": [
            "Полный доступ к мощному GPT-ассистенту",
            "Большие лимиты для работы & творчества",
            "Премиум-скорость обработки",
            "Генерации изображений высокого качества"
        ],
        "pro_premium": [
            "Максимальные лимиты без компромиссов",
            "Приоритетные вычислительные мощности",
            "Идеально для бизнеса, маркетинга и потоковой работы",
            "Профессиональная генерация и редактирование изображений"
        ]
    }

    # Преобразуем технические лимиты в красивые строки
    req_limit = "∞" if plan.max_requests is None else f"до {plan.max_requests}"
    img_limit = "∞" if plan.max_image_generations is None else f"до {plan.max_image_generations}"
    text_limit = f"до {plan.max_text_len} символов"

    # Собираем «что входит»
    limits_block = (
        f"• 📝 Запросы: <b>{req_limit}</b>\n"
        f"• 🎨 Генерации изображений: <b>{img_limit}</b>\n"
        f"• ✍️ Макс. длина сообщения: <b>{text_limit}</b>\n"
    )
    ...
    text = (
        ...
        f"➡️ Идеальный выбор, если вам нужно больше свободы, удобства и скорости."
    )
