...
async def process_successful_payment(payment_info: Any, user_id: int, database, bot_instance) -> None:
    """
    ...
    try:
        ...
        if subscription_type:
            ...
            database.add_subscription(user_id, subscription_type_enum, duration_days)
            await send_subscription_confirmation(user_id, subscription_type_enum, database, bot_instance)
...
async def send_subscription_confirmation(user_id: int, subscription_type: SubscriptionType, database, bot_instance) -> None:
    """
    Отправляет подтверждение об активации подписки.
    """
    user = database.user_collection.find_one({"_id": user_id})
    if user and bot_instance:
...
async def check_pending_payments(database, bot_instance) -> None:
    ...
    try:
        ...
        for payment in pending_payments:
            ...
            try:
                ...
                if status == 'succeeded':
                    await process_successful_payment(payment_info, user_id, database, bot_instance)
