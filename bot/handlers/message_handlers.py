...
class MessageHandlers(MessageProcessor, PhotoEditorMixin, UserManager):
        ...
        try:
            async with user_semaphores[user_id]:
                ...
                if config.enable_message_streaming:
                    ...
                    await AIResponseHandler.edit_ai_response(context, placeholder_message, answer, chat_mode)
            ...
            if should_update and answer.strip():
                try:
                    await AIResponseHandler.edit_ai_response(
                        context, placeholder_message, answer[:4096], parse_mode
                    )
                    prev_answer = answer
                    last_update_time = current_time
                except Exception:
                    # Игнорируем ошибки редактирования при потоковой передаче
                    pass
    ...
    async def _vision_message_handle_fn(self, update: Update, context: CallbackContext,
        ...
        try:
            ...
            async for gen_item in gen:
                ...
                try:
                    await AIResponseHandler.edit_ai_response(
                        context, placeholder_message, answer, parse_mode
                    )
                except Exception:
    ...
    async def _transcribe_voice_message(self, update: Update, context: CallbackContext, chat_mode: str) -> str:
        ...
        if chat_mode == "stenographer":
            ...
            await AIResponseHandler.edit_ai_response(
                context, placeholder_message, transcription_message
            )
        else:
            await AIResponseHandler.edit_ai_response(
                context, placeholder_message, text
            )
