...
🎨 Генерация изображений по текстовым запросам в режиме <b>👩‍🎨 Художник</b> /mode?code=art
⌨️ Создание расшифровок голосовых сообщений в режиме <b>⌨️ Стенограф</b> /mode?code=message_audio
...
2. <b>Ассистент</b> - это режим по умолчанию. Экспериментируйте с другими <b>режимами</b> и посмотрите, какой подходит вам лучше всего! /mode
...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = "👋 Привет — я <b>Ducks GPT</b> 🤖 — компактный чат-бот на базе <b>ChatGPT</b>. Рад знакомству!\n\n"
    reply_text += "Почему я крутой:\n"
    reply_text += "• Доступен в РФ.\n"
    reply_text += "• Платишь только за то, что используешь — без месячной подписки.\n"
    reply_text += "• Нет лимита сообщений в час: общаемся пока на балансе (мин. <b>€1.25</b>).\n"
    reply_text += "🎁 Первые <b>100 ₽</b> за мой счёт!\n\n"
    reply_text += "Я в разработке — буду рад <b>фидбеку</b>.\n\n"
    reply_text += "Команды:\n"
    reply_text += "• /new — новый диалог\n"
    reply_text += "• /retry — перегенерировать ответ\n"
    reply_text += "• /mode — выбрать режим (Art — генерация изображений, Stenograph — расшифровка голосов)\n"
    reply_text += "• /balance — баланс • /topup — пополнить\n"
    reply_text += "• /settings • /help • /role\n\n"
    reply_text += "Дополнительно:\n"
    reply_text += "• Отправляй голосовые сообщения — я их распознаю.\n"
    reply_text += "• Добавить в группу: /help_group_chat\n\n"
    reply_text += "Готовы? Напишите что-нибудь — поболтаем! 🦆✨\n\n"
    reply_text += f"Если есть <b>вопросы</b> или <b>отзывы</b>, {'напиши моему разработчику: ' + developer_info if developer_info else ''}\n\n"
