...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = "👋 Привет! Я <b>Ducks GPT</b>, твой дружелюбный чат-бот по соседству. Рад знакомству! \n\n"
    reply_text += "     Я телеграм-бот 🤖 на базе <b>ChatGPT</b>, и я здесь, чтобы помочь с любыми твоими вопросами. \n\n"
    reply_text += "Ты можешь спросить себя:\n  <i><b>Зачем использовать этого бота, если можно просто открыть ChatGPT в браузере?</b></i> 🤔\n\n"
    reply_text += "  Всё просто: \n - я доступен в РФ \n - у меня система <b>пополнения баланса</b>, то есть ты платишь только за то, что используешь. Не нужно переживать из-за подписки на $20 в месяц!\n - у меня <b>нет лимита сообщений</b> в час. Пока у тебя есть хотя бы <b>€1.25</b> на балансе, мы можем болтать <b>сколько угодно!</b>\n Разве это не круто? 😎\n\n"
    reply_text += " 🤫 Тсс!\nТолько не говори моему создателю, но <b>первые 100 рублей — за мой счёт!</b> \n\n У тебя будет достаточно времени, чтобы решить, хочешь ли ты продолжать пользоваться мной и поддержать нас обоих. <b>Мы очень это ценим!</b> 🥰 \n\n"
